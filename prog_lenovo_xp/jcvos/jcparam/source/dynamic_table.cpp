...
void CDynaTab::AddColumn(LPCTSTR name)
{
	CColInfoBase * col_info = new CColInfoBase(
		m_col_info.GetSize(), 0, 0, name);
	m_col_info.AddItem(col_info);
}

//void CDynaTab::AddColumn(const CColInfoBase * col_info)
//{
//	JCASSERT(col_info);
//	col_info->m_id = m_col_info.GetSize();
//	m_col_info->AddItem(col_info);
//}

void CDynaTab::AddRow(CDynaRow * & row)
{
	JCASSERT(NULL == row);
...
void CDynaRow::GetColumnData(LPCTSTR name, IValue * & val) const
{
	//GetSubValue(field_name, val);
	JCASSERT(m_col_info);
	JCASSERT(NULL == val);
	const CColInfoBase * col_info = m_col_info->GetItem(name);
	if ( (!col_info) || (col_info->m_id >= m_col_size) )	
...
{
	JCASSERT(NULL == val);
	JCASSERT(field < m_col_size);
	val = m_cols[field];
	if (val) val->AddRef();
}

void CDynaRow::GetColumnData(const CColInfoBase * col, IValue * & val) const
{
	JCASSERT(col)
	JCASSERT(NULL == val);
	JCASSERT(col->m_id < m_col_size);
	val = m_cols[col->m_id];
	if (val) val->AddRef();
}
