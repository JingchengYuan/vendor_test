...
LOG_CLASS_SIZE_T( jcparam::CValue<int>, 0 )
LOG_CLASS_SIZE_T( jcparam::CValue<CJCStringT>, 1 )
...
jcparam::IValue * jcparam::CreateTypedValue(jcparam::VALUE_TYPE vt, void * data)
{
	...
	case jcparam::VT_CHAR:
		val = CValue<char>::Create( (data)?*((const char *)data):(0) );					
		break;
	case jcparam::VT_UCHAR:		val = CValue<unsigned char>::Create();			break;
	case jcparam::VT_SHORT:		val = CValue<short>::Create();					break;
	case jcparam::VT_USHORT:	val = CValue<unsigned short>::Create();			break;
	case jcparam::VT_INT:		val = CValue<int>::Create();					break;
	case jcparam::VT_UINT:		val = CValue<unsigned int>::Create();			break;
	case jcparam::VT_INT64:		val = CValue<INT64>::Create();				break;
	case jcparam::VT_UINT64:	val = CValue<UINT64>::Create();		break;
	case jcparam::VT_FLOAT:		val = CValue<float>::Create();					break;
	case jcparam::VT_DOUBLE:	val = CValue<double>::Create();					break;
	case jcparam::VT_STRING:	val = CValue<CJCStringT>::Create();				break;
	case jcparam::VT_BOOL:		val = CValue<bool>::Create();					break;
	default:
